...
async function getEntities(baseUrl, token) {
    ...
    const template = [
        ...
        "{% set friendly_name = states[entity].attributes.friendly_name or 'Sconosciuto' %}",
        ...
        "{% set ns.entities_list = ns.entities_list + [{'e': entity, 'f': friendly_name, 'a': area, 't': domain, 'd': device_name, 'df': device_friendly, 'desc': description}] %}",
        "{% endif %}",
        "{% endfor %}",
